...
export default class TripController {
  ...
  render() {
    this._shownEvents = this._events.get();
    let sortedDays = [];

    if (this._shownEvents.length) {
      this._sortComponent.setSortTypeChangeHandler((sortType) => {
        switch (sortType) {
          case SortType.TIME:
            sortedDays = sortEventsByTime(this._shownEvents);
            break;

          case SortType.PRICE:
            sortedDays = sortEventsByPrice(this._shownEvents);
            break;

          default:
          case SortType.DEFAULT:
            sortedDays = splitEventsByDay(this._shownEvents);
            break;
        }
      ...
      const days = splitEventsByDay(this._shownEvents);
