...
const createDestinationHtml = (destination) => {
  ...
  const photoList = generatePhotoList().map((item) => `<img class="event__photo" src="${item}" alt="Event photo">`).join(`\n`);
...
export const createForm = (eventItem = EVENT_DEFAULT) => {
  ...
  const startDateTime = `${getDate(eventItem.start, `/`)} ${getTime(eventItem.start)}`;
  const finishDateTime = `${getDate(eventItem.finish, `/`)} ${getTime(eventItem.finish)}`;
