...
const createEditEventTemplate = (event = defaultTrip) => {
  const isEditForm = event !== defaultTrip;
  ...
  const destination = event.desctination;
  const destinationList = cities.map((item) => `<option value="${item}"></option>`).join(`\n`);

  const startDateTime = `${getDate(event.start, `/`)} ${getTime(event.start)}`;
  const finishDateTime = `${getDate(event.finish, `/`)} ${getTime(event.finish)}`;
  ...
  return (
    `<form class="event  event--edit" action="#" method="post">
      ...
      <section class="event__details">
        ...
        ${createDestination(event.destination)}
      </section>
    </form>`);
};
